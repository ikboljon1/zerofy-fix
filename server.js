...
// Получен��е данных о рекламе товаров
...
app.post('/api/user-stores', (req, res) => {
  ...
  db.run(query, [
    ...
    if (err) {
      console.error('Error adding user store:', err);
      return res.status(500).json({ error: 'Ошибка при добавлении магазина пользователя' });
    }

    // Обновляем счетчик магазинов пользователя
    db.run('UPDATE users SET store_count = (SELECT COUNT(*) FROM user_stores WHERE user_id = ?) WHERE id = ?', 
      [userId, userId], function(updateErr) {
        if (updateErr) {
          console.error('Error updating store count:', updateErr);
          // Продолжаем выполнение, так как магазин уже добавлен
        }
...
// Обновление выбора магазина пользователем
app.put('/api/user-stores/:storeId/select', (req, res) => {
  const { storeId } = req.params;
  ...
  db.run('UPDATE user_stores SET is_selected = 0 WHERE user_id = ?', [userId], function(err) {
    if (err) {
      ...
      return res.status(500).json({ error: 'Ошибка при обновлении выбора магазина' });
    }

    // Затем устанавливаем выбор для указанного магазина
    db.run('UPDATE user_stores SET is_selected = 1 WHERE user_id = ? AND store_id = ?', [userId, storeId], function(updateErr) {
      if (updateErr) {
        console.error('Error updating store selection:', updateErr);
        return res.status(500).json({ error: 'Ошибка при обновлении выбора магазина' });
      }
...
// Обновление даты последней выборки для магазина
app.put('/api/user-stores/:storeId/update-fetch-date', (req, res) => {
  const { storeId } = req.params;
  const { lastFetchDate } = req.body;
  
  if (!lastFetchDate) {
    return res.status(400).json({ error: 'Необходимо указать lastFetchDate' });
  }

  db.run('UPDATE user_stores SET last_fetch_date = ? WHERE store_id = ?', [lastFetchDate, storeId], function(err) {
    if (err) {
      console.error('Error updating last fetch date:', err);
      return res.status(500).json({ error: 'Ошибка при обновлении даты последней выборки' });
    }
...
app.delete('/api/user-stores/:storeId', (req, res) => {
  ...
  db.run('DELETE FROM user_stores WHERE store_id = ? AND user_id = ?', [storeId, userId], function(err) {
    if (err) {
    ...
    db.run('UPDATE users SET store_count = (SELECT COUNT(*) FROM user_stores WHERE user_id = ?) WHERE id = ?', 
      [userId, userId], function(updateErr) {
        if (updateErr) {
          console.error('Error updating store count:', updateErr);
          // Продолжаем выполнение, так как магазин уже удален
        }
...
// API для получения настроек приложения
app.get('/api/app-settings', (req, res) => {
  db.all('SELECT * FROM app_settings', (err, rows) => {
    if (err) {
      console.error('Error getting app settings:', err);
      return res.status(500).json({ error: 'Ошибка при получении настроек приложения' });
    }

    // Преобразуем массив объектов в один объект с ключами и значениями
    const settings = {};
    rows.forEach(row => {
...
// API для обновления настроек приложения
app.put('/api/app-settings', (req, res) => {
  const settings = req.body;
  
  if (!settings || Object.keys(settings).length === 0) {
    return res.status(400).json({ error: 'Необходимо указать настройки для обновления' });
  }

  // Для каждой настройки выполняем запрос на обновление или вставку
  const promises = Object.entries(settings).map(([key, value]) => {
    return new Promise((resolve, reject) => {
      db.run(
        'INSERT OR REPLACE INTO app_settings (setting_name, setting_value, last_updated) VALUES (?, ?, CURRENT_TIMESTAMP)',
        [key, value],
        function(err) {
          if (err) {
            reject(err);
  ...
  Promise.all(promises)
    ...
    .catch(err => {
      ...
      res.status(500).json({ error: 'Ошибка при обновлении настроек приложения' });
    });
});

// API для тестирования подключения к SMTP
app.post('/api/test-smtp-connection', (req, res) => {
  const { smtp_host, smtp_port, smtp_user, smtp_password } = req.body;
  
  if (!smtp_host || !smtp_port || !smtp_user || !smtp_password) {
    return res.status(400).json({ error: 'Все поля настроек SMTP обязательны' });
  }
  ...
  transporter.verify((err, success) => {
    ...
    res.json({ success: true, message: 'Подключение к SMTP серверу успешно установлено' });
  });
});

// Тестирование соединения с POP3
app.post('/api/test-pop3-connection', (req, res) => {
  const { pop3_host, pop3_port, pop3_user, pop3_password } = req.body;
  
  if (!pop3_host || !pop3_port || !pop3_user || !pop3_password) {
    return res.status(400).json({ error: 'Все поля настроек POP3 обязательны' });
  }

  // Мы можем только вернуть успех, так как требуется отдельная библиотека для POP3
  // В реальном сценарии вы можете использовать библиотеку, такую как 'poplib' или 'node-pop3'
  res.json({ success: true, message: 'Настройки POP3 сохранены' });
});

// API для управления настройками верификации
app.get('/api/verification-settings', (req, res) => {
  db.get("SELECT setting_value FROM app_settings WHERE setting_name = 'verification_method'", (err, row) => {
    if (err) {
      console.error('Error getting verification settings:', err);
      return res.status(500).json({ error: 'Ошибка при получении настроек верификации' });
    }

    // По умолчанию используем email, если настройка не найдена
    const method = row ? row.setting_value : 'email';
    res.json({ method });
  });
});

app.put('/api/verification-settings', (req, res) => {
  const { method } = req.body;
  
  if (!method || (method !== 'email' && method !== 'phone')) {
    return res.status(400).json({ error: 'Метод верификации должен быть email или phone' });
  }
  ...
  db.run(
    ...
    function(err) {
      if (err) {
        console.error('Error updating verification settings:', err);
        return res.status(500).json({ error: 'Ошибка при обновлении настроек верификации' });
      }
